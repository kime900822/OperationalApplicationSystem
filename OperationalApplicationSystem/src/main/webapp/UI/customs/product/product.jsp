...
function customsProductHandingOK(){
	...
	BJUI.ajax('doajax', {
	    url: 'customs/productHandingOK.action',
	    loadingmask: true,
...
function customsProductHandingNO(){
	...
	BJUI.ajax('doajax', {
	    url: 'customs/productHandingNO.action',
	    loadingmask: true,
	    data:{batchNumber:batchNumber},	    
	    okCallback: function(json, options) {
            if(json.status='200'){
...
<div class="bjui-pageContent" id="div-user">
    <table class="table table-bordered" id="datagrid-customs-product-filter" data-toggle="datagrid" data-options="{
        ...
        exportOption: {type:'file', options:{url:'customs/exportCustomsProduct.action', loadingmask:true,queryForm:$.CurrentNavtab.find('#datagrid-customs-product-query')}}
    }">
        <thead>
            <tr>
