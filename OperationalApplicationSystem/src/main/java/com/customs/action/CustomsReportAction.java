	...
	public String getReport1SQl() throws Exception {
       	...
       	String where=" where 1=1 ";
    		...
    		String sql=" select B.`no` AS columen1,A.`no` AS columen2 ,'1' AS columen3,"
    				+ " ROUND(sum(A.quantityIssued)/1000.0*(case when C.declareUnitCode='030' then 1 else 1000.0 end),3) AS columen4,"
    				+ " case when C.declareUnitCode='030' then '3' else '0' end AS columen5 ,"
    				+ "'0' AS  columen6 ,"
    				+ "'' AS columen7,"
    				+ "'100' AS columen8 ,"
    				+ "'3' AS columen9 ,"
