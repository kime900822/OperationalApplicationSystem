...
public class CustomsGeneralBIZImpl  extends BizBase implements CustomsGeneralBIZ{
	...
	public String getSQL(String month,String where) throws Exception {
		...
		String sql=" select total.* from (         "
				...
				+" ROUND(sum(t2.quantityIssued)*(1+(case when t3.declareUnitCode='030' then 0.03 else 0 end))/(case when t3.declareUnitCode='030' then 1000.0 else 1.0 end),2) as WriteOffVolume,                                                                               "
