...
public class CustomsMaterialBIZImpl extends BizBase implements CustomsMaterialBIZ{
	...
	public List getByNewMaterialNo(String materialNo) {
		return customsMaterialDAO.query(" where materialNo=IFNULL((select oldMaterialNo from CustomsMaterialMapping where newMaterialNo='"+materialNo+"' and oldMaterialNo not like'%Lynn'),'"+materialNo+"') ");         
	}
